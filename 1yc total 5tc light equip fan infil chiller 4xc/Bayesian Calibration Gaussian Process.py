...
def Bayesian_Calibration(DataComp,DataField,DataPred,output_folder):
  ...
  with pm.Model() as model:
    # Claim prior part
    eta = pm.HalfCauchy("eta", beta=5) # for eta of gaussian process
...
# Resouce file
folder = './'
DataComp = np.asarray(pd.read_csv("./DATACOMP_Single.csv"))
DataField = np.asarray(pd.read_csv("./DATAFIELD_Single.csv"))[:12,:]
DataPred = np.asarray(pd.read_csv("./DATAFIELD_Single.csv"))[12:,:]
